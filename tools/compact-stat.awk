...
BEGIN{ORS="\t"; start=-1; end=-1; minor_start=-1; minor_end=-1;
	major_c=0; minor_c=0; major_mv_c=0; 
	major_byte=0; minor_byte=0; major_mv_byte=0;
	major_fnum=0; gap=0; 
	work=0; minor_work=0; major_mv_work=0; }
{
	if (match("Compacting", $3)>0) {
		...
		if (blevel > 0) {
			...
			major_fnum=major_fnum + ulevel + blevel;
			major_c = major_c + 1;
		} else {
...
END{work = work/1000; minor_work=minor_work/1000; major_mv_work=major_mv_work/1000; gap=gap/1000;
	major_mb = major_byte/1048576; minor_mb=minor_byte/1048576; major_mv_mb=major_mv_byte/1048576;
if(title) printf("#major #minor #mv #file\tmajor.s mionr.s mv.s\tthoughput file.s.avg\tgap major.t minor.t mv.t\tmajor.t.avg major.s.avg\n");
print major_c, minor_c, major_mv_c, major_fnum, "\t",
	major_mb, minor_mb, major_mv_mb, "\t",
	(2*major_mb+2*minor_mb)/(work+minor_work), major_mb/major_fnum,"\t",
	gap, work, minor_work, major_mv_work;

printf("\t%.3f %.3f\t", work/major_c, major_mb/major_c) 
}
