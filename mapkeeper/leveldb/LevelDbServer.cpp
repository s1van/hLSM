...
public:
	...
	const leveldb::FilterPolicy* filter_policy_ = leveldb::NewBloomFilterPolicy(20);

        options.max_open_files = 64000;
    ...
    void scanAscending(RecordListResponse& _return, leveldb::DB* db, 
        ...
        int numBytes = 0;
        leveldb::Iterator* itr = db->NewIterator(leveldb::ReadOptions(), true);
        _return.responseCode = ResponseCode::ScanEnded;
...
int main(int argc, char **argv) {
    ...
    config.add_options()
        ...
        ("write-buffer-mb,w", po::value<int>(&writeBufferSizeMb)->default_value(8), "LevelDB write buffer size in MB")
        ("block-cache-mb,b", po::value<int>(&blockCacheSizeMb)->default_value(1024), "LevelDB block cache size in MB")
        ("bloom-bits-use,B", po::value<int>(&hlsm::config::bloom_bits_use)->default_value(10), "#bits used in bloom filter")
        ("hlsm-mode,H", po::value<std::string>(&mode)->default_value("Default"), "hLSM-tree mode")
    ...
    hlsm::config::mode.set(mode.c_str());
    hlsm::config::secondary_storage_path = sec_dir.c_str();
    hlsm::config::primary_storage_path = dir.c_str();
    fprintf(stderr, "store: %s, %s\n", dir.c_str(), hlsm::config::secondary_storage_path);
    leveldb::config::kTargetFileSize = 8 * 1048576;
